       ...
       */
        ...
        */
        public void ExecXML()
        {
            ...
            if (_cakeSettings.CAKE_MCRYPT_ALG == "base64")
            {
                var plainTextBytes = System.Text.Encoding.UTF8.GetBytes(xml);
                xml_req = System.Convert.ToBase64String(plainTextBytes);
            }
            else
            {
                cakeCrypt = new CakeCrypt(_cakeSettings.CAKE_MCRYPT_ALG, _cakeSettings.CAKE_MCRYPT_MODE, _cakeSettings.CAKE_INTERFACE_KEY);

                cakeCrypt.cryptor.IV = Encoding.Default.GetBytes("00000000");
                xml_req = cakeCrypt.CakeEncryptHex(xml);
            ...
            using (WebClient webClient = new WebClient())
            {
                ...
                if (_cakeSettings.CAKE_MCRYPT_ALG == "base64")
                {
                    data.Add("request", xml_req);
                    CakeDebug.AddMessage(xml_req, _cakeSettings);
                }
                else
                {
